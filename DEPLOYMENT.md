...
**Live at [sosdz.org](https://sosdz.org).** Deployed on a Contabo VPS, Ubuntu 24.04, per the steps below. The steps below are the exact, copy-paste commands for standing up the whole app (backend + frontend + moderation sidecar) on one VPS with Nginx as the single public entry point -- kept here both as the historical record of that setup and for redeploying/rebuilding a replacement VPS if ever needed. They're written to be run by whoever holds SSH access to that server -- an AI session has no path to execute them itself (no SSH tool, and this kind of sandboxed session's outbound network is HTTPS-only through a policy proxy that doesn't carry raw SSH traffic). Run each block yourself over SSH, in order; paste back any error output for help debugging it. A Railway/Render fallback is documented below as an alternative if VPS setup is ever blocking progress. See "Redeploying on a new push" (step 6) for the routine update flow on the already-live server.
...
| `GEMINI_FALLBACK_MODELS` | `gemini-3.5-flash-lite,gemini-2.5-flash,gemini-2.5-pro` | comma-separated, tried in order only after `GEMINI_MODEL` exhausts its own retries on a transient "high demand" 503 -- each is a separate model with its own free-tier capacity, so it isn't necessarily under the same load. A model ID Google later deprecates just gets skipped in favor of the next one in the list (confirmed live), so this list doesn't need to be perfect, only roughly current; leave blank to just fail the submission when `GEMINI_MODEL` is unavailable, same as before this existed |
