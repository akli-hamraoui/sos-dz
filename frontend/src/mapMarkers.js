...
export function attachMapPopupBehavior(map, onPhoto) {
  ...
  // Leaflet fires "moveend" after panBy(). If the popup-centering handler
  // responds to that same event without a guard, it pans again because the
  // popup is anchored above its marker and can never have its DOM center at
  // exactly the same pixel as the map center. That creates a feedback loop
  // of tiny animated pans; on mobile this can leave the tile layer looking
  // completely gray while the map keeps moving.
  // Keep the recenter behavior, but make the centering pan synchronous and
  // explicitly ignore the moveend generated by our own pan.
  const centerPopup = (popup, state) => {
  ...
  const onOpen = (e) => {
    ...
    const state = { isCentering: false }
    const recenter = () => {
      // Ignore the moveend generated by our own synchronous panBy().
      if (state.isCentering) {
        state.isCentering = false
        return
      }
      centerPopup(popup, state)
    }
